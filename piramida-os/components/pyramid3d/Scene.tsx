...
function Content({ interactive, highlight, recommendMode, bare }: { interactive: boolean; highlight?: Set<string>; recommendMode?: boolean; bare?: boolean }) {
  const view = usePyramid((s) => s.view);
  const floorId = usePyramid((s) => s.floorId);
  const spaceId = usePyramid((s) => s.spaceId);
  ...
  if (view === "floor" && floorId != null)
    return (
      <group key={`floor:${floorId}`}>
        <FloorSpaces floorId={floorId} interactive={interactive} highlight={highlight} recommendMode={recommendMode} />
      </group>
    );
...
export default function Scene({ interactive = true, autoRotate = false, highlight, recommendMode = false, bare = false }: SceneProps) {
  ...
  return (
    ...
    >
      ...
      <Suspense fallback={null}>
        <Content interactive={interactive} highlight={highlightSet} recommendMode={recommendMode} bare={bare} />
      </Suspense>
