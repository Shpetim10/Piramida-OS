  ...
  return (
    <group>
      ...
      {floor.spaces.map((space, i) => {
        ...
        return space.stairs ? (
          <StairsSpace key={space.id} space={space} index={i} interactive={interactive} recommended={recommended} recommendMode={recommendMode} recColor={recColor} onSelect={() => selectSpace(space.id)} />
        ) : (
          <SpaceCube key={space.id} space={space} index={i} lift={lifted ? STORY : 0} interactive={interactive} recommended={recommended} recommendMode={recommendMode} recColor={recColor} onSelect={() => selectSpace(space.id)} />
        );
      })}
    </group>
  );
...
function StairsSpace({ space, onSelect, index = 0, interactive = true, recommended = false, recommendMode = false, recColor = ACCENT }: { space: EventSpace; onSelect: () => void; index?: number; interactive?: boolean; recommended?: boolean; recommendMode?: boolean; recColor?: string }) {
...
function SpaceCube({ space, onSelect, lift = 0, index = 0, interactive = true, recommended = false, recommendMode = false, recColor = ACCENT }: { space: EventSpace; onSelect: () => void; lift?: number; index?: number; interactive?: boolean; recommended?: boolean; recommendMode?: boolean; recColor?: string }) {
  const [hover, setHover] = useState(false);
  const rad = (space.angle * Math.PI) / 180;
  const x = Math.cos(rad) * space.radius;
  const z = Math.sin(rad) * space.radius;
