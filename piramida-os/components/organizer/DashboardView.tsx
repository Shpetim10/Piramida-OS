...
import type { OrganizerDashboard } from "@/lib/organizer/portal-data";
...
export function DashboardView({
  ...
  activeEvent,
  requests,
}: { name: string } & OrganizerDashboard) {
      ...
      >
          ...
          {activeEvent ? (
            <>
              <div style={{ position: "relative", font: "600 10px 'JetBrains Mono', monospace", color: "#C8F000", letterSpacing: ".16em", marginBottom: 6 }}>
                ACTIVE EVENT · {activeEvent.status.toUpperCase()}
              </div>
            ...
            <>
              ...
              <p style={{ position: "relative", font: "400 14px/1.55 Inter, sans-serif", color: "#AEB5C2", maxWidth: 380, margin: "0 0 22px" }}>
                Describe what you want to host and the Pyramid will recommend spaces and build a live quote.
              </p>
              <Link href="/organizer/create" style={{ position: "relative", display: "inline-block", padding: "12px 20px", borderRadius: 11, background: "#C8F000", color: "#0D0D12", font: "700 13px Inter, sans-serif", textDecoration: "none" }}>
        ...
        <div style={{ border: "1px solid rgba(255,255,255,.07)", borderRadius: 20, background: "#151821", padding: 22 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div style={{ font: "600 10px 'JetBrains Mono', monospace", color: "#7D8799", letterSpacing: ".16em" }}>RECENT REQUESTS</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {requests.length === 0 ? (
              <div style={{ font: "400 13px Inter, sans-serif", color: "#7D8799" }}>No requests submitted yet.</div>
            ) : (
              requests.map((r) => (
                <div key={r.id} style={{ border: "1px solid rgba(255,255,255,.06)", borderRadius: 13, padding: 14 }}>
